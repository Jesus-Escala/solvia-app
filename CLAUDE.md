...
- **Locations** (`inventory` module, `LocationsPage` at `/locations`, `GET/POST /maps`…): floor plans of the business (a picture, uploaded shrunk by `lib/planImage.ts`) with spots (`x`/`y` as fractions, a theme color) and the products kept in each (`Product.spotId`). `components/maps/MapStage` draws the plan in 2D or 3D (CSS 3D transforms, pins stand up; drag to pan/turn, wheel or pinch to zoom; in 2D a tap marks a spot and pins drag to move). "¿Dónde está?" search highlights spots by name or product; the product form has a "Ubicación" select and the POS product detail shows it with "Ver en el plano" (`SpotMapModal`).
