...
import { AXIS_TICK, ChartTooltipCard, cx, useChartColors } from '@/ui';
...
export function PeriodChart({
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 bg-surface-2 text-[11px] tracking-wide text-muted uppercase">
            <tr>
              ...
              {SERIES.map((item) => (
                <th key={item.key} className="px-3 py-2 text-right font-semibold whitespace-nowrap">
                  {item.label}
                </th>
              ))}
          ...
          <tbody>
            {[...series].reverse().map((point) => (
              <tr key={point.bucket} className="border-t border-line">
                ...
                <td className="px-3 py-2 text-right tabular-nums">{fmt.money(point.collected)}</td>
                ...
                <td className="px-3 py-2 text-right tabular-nums">{fmt.number(point.payments)}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="h-[280px] w-full" role="img" aria-label={t('dashboard.analytics.chartTitle')}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={series} margin={{ top: 8, right: 4, bottom: 0, left: 0 }} barGap={2}>
