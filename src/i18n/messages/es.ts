import { uiEs } from '@/ui';

/**
 * Spanish (default) UI strings. This file is the source of truth: `en.ts` must have exactly the
 * same shape (enforced by the `Messages` type). Use `{name}` placeholders for interpolation.
 */
export const es = {
  ...uiEs,
  common: {
    appName: 'Solvia',
    tagline: 'Anota lo que fías y cobra a tiempo',
    loading: 'Cargando...',
    save: 'Guardar',
    cancel: 'Cancelar',
    close: 'Cerrar',
    edit: 'Editar',
    delete: 'Eliminar',
    search: 'Buscar',
    actions: 'Acciones',
    back: 'Volver',
    all: 'Todos',
    retry: 'Reintentar',
    refresh: 'Actualizar',
    view: 'Ver',
    viewAll: 'Ver todo',
    dismiss: 'Descartar',
    optional: 'opcional',
    updatedJustNow: 'Actualizado justo ahora',
    updatedMinutesAgo: 'Actualizado hace {minutes} min',
    moreActions: 'Más acciones',
  },
  nav: {
    section: 'Menú',
    home: 'Inicio',
    dashboard: 'Dashboard',
    reports: 'Reportes',
    products: 'Productos',
    sales: 'Ventas',
    more: 'Más',
    purchases: 'Compras',
    suppliers: 'Proveedores',
    locations: 'Ubicaciones',
    groups: {
      commercial: 'Comercial',
      receivables: 'Cuentas por cobrar',
      inventory: 'Inventario',
      numbers: 'Resultados',
    },
    customers: 'Clientes',
    receivables: 'Cobranza',
    settings: 'Configuración',
    help: 'Ayuda',
    collapse: 'Contraer menú',
    expand: 'Expandir menú',
    openMenu: 'Abrir menú',
    closeMenu: 'Cerrar menú',
    footer: '© {year} Solvia',
  },
  topbar: {
    help: 'Ayuda y recorrido guiado',
    account: 'Mi cuenta',
    logout: 'Cerrar sesión',
    changePassword: 'Cambiar contraseña',
    startTour: 'Recorrido guiado',
    helpCenter: 'Centro de ayuda',
  },
  quick: {
    purchase: 'Nueva compra',
    purchaseHint: 'Te llegó mercadería: sube el stock',
    sale: 'Nueva venta',
    saleHint: 'Cobra o fía con tus productos',
    button: 'Anotar',
    receivable: 'Anotar fiado',
    receivableHint: 'Alguien se llevó algo y te paga después',
    customer: 'Nuevo cliente',
    customerHint: 'Agrega a alguien que te compra fiado',
    paymentHint: 'Alguien te pagó todo o una parte',
    payment: {
      label: 'Registrar pago',
      title: 'Registrar un pago',
      who: '¿Quién te pagó?',
      which: '¿Qué deuda te pagó?',
      back: 'Volver',
      nothingOwed: '{name} no te debe nada.',
    },
  },
  picker: {
    placeholder: 'Escribe el nombre del cliente…',
    owes: 'Debe {amount}',
    change: 'Cambiar',
    create: 'Nuevo cliente: «{name}»',
    noResults: 'No hay clientes con ese nombre.',
    typeToSearch: 'Escribe para buscar.',
  },
  home: {
    salesMonth: 'Vendiste este mes',
    salesMonthCount: '{count|# venta|# ventas}',
    salesLink: 'Ver mis ventas',
    stockValue: 'Tu inventario vale',
    stockValueHint: '{count|# producto|# productos} (a precio de costo)',
    stockLink: 'Ver mis productos',
    lowStock: '{count|# producto se está acabando|# productos se están acabando}',
    lowStockHint: 'Revísalos y compra antes de que te falten.',
    greeting: 'Hola, {name}',
    subtitle: '¿Qué quieres hacer hoy?',
    owed: 'Te deben',
    owedCount: '{count|# deuda por cobrar|# deudas por cobrar}',
    owedLate: '{amount} ya están atrasados',
    owedLink: 'Ver quién te debe',
    paidMonth: 'Te pagaron este mes',
    paidMonthCount: '{count|# pago|# pagos}',
    paidLink: 'Ver cómo te pagaron',
    today: {
      title: 'Para cobrar ahora',
      subtitle: 'Los atrasados y los que vencen en los próximos 3 días.',
      empty: '¡Nadie atrasado!',
      emptyDescription: 'No tienes deudas atrasadas ni que venzan en los próximos días.',
      more: 'Ver {count|# más|# más}',
    },
    empty: {
      title: 'Aún no anotaste ningún fiado',
      description:
        'Anota lo primero que te deben: el nombre de la persona, cuánto y para cuándo te paga. Solvia se encarga de recordárselo.',
    },
    reports: {
      title: '¿Quieres ver más números?',
      hint: 'En Reportes ves tus ventas y cobros por cliente, lo que más vendes y tu stock.',
    },
  },
  viewer: {
    preparing: 'Preparando el archivo…',
    cannotPreview: 'No se pudo mostrar la vista previa. Descárgalo para abrirlo.',
    downloadPdf: 'Descargar PDF',
    downloadExcel: 'Descargar Excel',
    zoomIn: 'Acercar',
    zoomOut: 'Alejar',
    fit: 'Ajustar al ancho',
    emptySheet: 'El archivo no tiene datos.',
    size: '{rows} filas · {columns} columnas',
    truncated: 'Vista previa de las primeras {rows} filas; descárgalo para verlo completo.',
  },
  reports: {
    title: 'Reportes',
    subtitle:
      'Tablas con el detalle de tu negocio. Elige un reporte y las fechas; puedes bajarlo a Excel.',
    choose: 'Elige un reporte',
    area: 'Área de los reportes',
    search: 'Buscar en el reporte',
    groups: {
      sales: 'Ventas',
      purchases: 'Compras',
      collections: 'Cobranza',
      inventory: 'Inventario',
    },
    names: {
      'sales-detail': 'Tickets (detalle de ventas)',
      'sales-by-day': 'Ventas por día',
      'sales-by-customer': 'Mejores clientes',
      'sales-by-product': 'Más vendidos',
      'sales-by-category': 'Por categoría',
      'sales-by-method': 'Por método de pago',
      'sales-by-seller': 'Por vendedor',
      'purchases-detail': 'Detalle de compras',
      'purchases-by-supplier': 'Por proveedor',
      'purchases-by-product': 'Productos comprados',
      'collections-by-customer': 'Cobros por cliente',
      stock: 'Stock y valor',
      shortages: 'Vendido sin stock',
    },
    hints: {
      'sales-detail':
        'Cada venta con su hora, cliente, productos, cómo pagó, descuento, lo que debe y quién la hizo (las anuladas también, marcadas).',
      'sales-by-day':
        'Cuánto vendiste cada día: cuántas ventas, al contado, fiado y ticket promedio.',
      'sales-by-category': 'Qué categorías venden más y cuánto ganas en cada una.',
      'sales-by-method':
        'Cuánto entró en efectivo, Yape, Plin, transferencia y cuánto se vendió fiado.',
      'sales-by-seller': 'Cuánto vendió cada usuario de tu equipo.',
      'purchases-detail':
        'Cada compra con su proveedor, productos, cómo pagaste, comprobante y quién la registró.',
      'purchases-by-supplier': 'Cuánto le compraste a cada proveedor y cuándo fue la última vez.',
      'purchases-by-product':
        'Qué productos compraste, cuánto gastaste, su costo promedio y el último costo.',
      'sales-by-customer':
        'Cuánto te compró cada cliente en esas fechas (sin contar ventas anuladas).',
      'sales-by-product': 'Qué productos vendiste, cuántos y cuánto ganaste (con el costo de hoy).',
      'collections-by-customer': 'Cuánto te pagó cada cliente de lo que te debía, y por qué medio.',
      stock: 'Lo que tienes hoy: cuánto queda, cuánto vale y qué se acabó o se está acabando.',
      shortages:
        'Cada vez que vendiste algo sin tener stock: cuándo, en qué venta y cuántos faltaban.',
    },
    emptyHints: {
      'sales-by-customer': 'No hay ventas en esas fechas. Prueba con otro periodo.',
      'sales-by-product': 'No hay ventas en esas fechas. Prueba con otro periodo.',
      'collections-by-customer': 'No registraste pagos en esas fechas. Prueba con otro periodo.',
      stock: 'Aún no tienes productos que lleven la cuenta de su stock.',
      shortages: '¡Bien! En esas fechas no vendiste nada sin stock.',
    },
    empty: 'Nada que mostrar',
    viewPdf: 'Ver PDF',
    viewExcel: 'Ver Excel',
    voided: 'Anulada',
    columns: {
      customer: 'Cliente',
      product: 'Producto',
      sold: 'Vendido',
      sales: 'Ventas',
      inSales: 'En ventas',
      cash: 'Al contado',
      credit: 'Fiado',
      lastSale: 'Última venta',
      collected: 'Te pagó',
      payments: 'Pagos',
      lastPayment: 'Último pago',
      quantity: 'Cantidad',
      cost: 'Costo',
      profit: 'Ganas',
      status: 'Estado',
      stock: 'Quedan',
      minStock: 'Avisar con',
      unitCost: 'Costo c/u',
      value: 'Vale (a costo)',
      retail: 'Vale (a precio de venta)',
      when: 'Cuándo',
      missing: 'Faltaban',
      soldUnits: 'Vendiste',
      balanceAfter: 'Stock quedó en',
      sale: 'Venta',
    },
    kpis: {
      sold: 'Total vendido',
      sales: 'Ventas',
      collected: 'Total cobrado',
      payments: 'Pagos',
      customers: 'Clientes que pagaron',
      cost: 'Costo',
      profit: 'Ganancia',
      value: 'Tu stock vale (a costo)',
      retail: 'Si lo vendes todo',
      out: 'Sin stock',
      low: 'Por acabarse',
      lines: 'Veces que vendiste sin stock',
      units: 'Unidades que faltaban',
    },
    stockStatus: {
      out: 'Sin stock',
      low: 'Por acabarse',
      ok: 'Bien',
    },
  },
  roles: {
    admin: 'Administrador',
    collector: 'Cobrador',
  },
  auth: {
    heroTitle: 'Tu negocio en orden, desde tu celular.',
    heroSubtitle: 'Vende, cobra lo que fías y controla tu stock en un solo lugar.',
    heroPoint1: 'Recordatorios de cobro por WhatsApp, solos',
    heroPoint2: 'Ventas con código de barras y stock al día',
    heroPoint3: 'Reportes en PDF y Excel sin salir de Solvia',
    signInTitle: 'Inicia sesión',
    signInSubtitle: 'Ingresa con tu correo y contraseña.',
    email: 'Correo electrónico',
    password: 'Contraseña',
    signIn: 'Ingresar',
    noAccount: '¿Aún no tienes cuenta?',
    requestAccess: 'Pruébalo gratis',
    changeTitle: 'Crea tu nueva contraseña',
    changeSubtitle: 'Por seguridad, reemplaza la contraseña temporal que te dieron.',
    changeSubtitleVoluntary: 'Actualiza la contraseña de tu cuenta.',
    currentPassword: 'Contraseña actual',
    temporaryPassword: 'Contraseña temporal',
    newPassword: 'Nueva contraseña',
    confirmPassword: 'Repite la nueva contraseña',
    passwordMismatch: 'Las contraseñas no coinciden.',
    changeSubmit: 'Guardar y continuar',
    changeDone: 'Tu contraseña se actualizó.',
    signOutInstead: 'Salir',
    back: 'Volver',
    registerLink: 'Registra tu negocio',
    registerTitle: 'Registra tu negocio',
    registerSubtitle: 'Crea tu cuenta de Solvia en un minuto.',
    businessName: 'Nombre del negocio',
    industry: 'Tipo de negocio',
    industryPlaceholder: 'Ej. tienda, restaurante, taller, servicios…',
    yourName: 'Tu nombre',
    createWorkspace: 'Crear mi cuenta',
    haveAccount: '¿Ya tienes cuenta?',
    signInLink: 'Inicia sesión',
    demoAccount: 'Cuenta demo',
    tabSignIn: 'Iniciar sesión',
    tabRegister: 'Crear cuenta',
    features: {
      label: 'Lo que Solvia hace por ti',
      remindersTitle: 'Recordatorios automáticos',
      remindersBody:
        'Solvia le escribe a tu cliente por WhatsApp antes y después de la fecha de pago.',
      paymentsTitle: 'Pagos con foto',
      paymentsBody: 'Anota si te pagó todo o una parte y guarda la captura de Yape o Plin.',
      riskTitle: '¿Quién paga puntual?',
      riskBody: 'Sabes quién paga a tiempo y quién suele atrasarse.',
      cashTitle: 'Lo que vas a cobrar',
      cashBody: 'Mira cuánta plata debería entrar esta semana.',
    },
    google: 'Continuar con Google',
    showPassword: 'Mostrar contraseña',
    hidePassword: 'Ocultar contraseña',
    orContinueWith: 'o continúa con tu correo',
    googleUnavailable: 'Por ahora entra con tu correo y contraseña.',
    googleCompleteTitle: 'Completa tu registro',
    googleCompleteSubtitle: 'Entrarás con {email}. Solo falta el nombre de tu negocio.',
    googleCreate: 'Crear mi cuenta',
    googleCancel: 'Usar otra cuenta',
  },
  status: {
    pending: 'Pendiente',
    partial: 'Pagó una parte',
    paid: 'Pagada',
    overdue: 'Atrasada',
  },
  risk: {
    label: 'Riesgo',
    low: 'Paga bien',
    medium: 'A veces se atrasa',
    high: 'Se atrasa mucho',
    noHistory: 'Todavía no hay pagos',
    details:
      'Paga a tiempo: {rate} · Se atrasa en promedio {days|# día|# días} · Atrasadas: {count}',
  },
  methods: {
    yape: 'Yape',
    plin: 'Plin',
    cash: 'Efectivo',
    bank_transfer: 'Transferencia',
  },
  /** Short names for tight rows of methods. */
  methodsShort: {
    yape: 'Yape',
    plin: 'Plin',
    cash: 'Efectivo',
    bank_transfer: 'Transf.',
  },
  templateTypes: {
    pre_due_reminder: {
      title: 'Aviso antes de la fecha',
      description: 'Se envía unos días antes de la fecha de pago.',
    },
    due_reminder: {
      title: 'Aviso el día de pago',
      description: 'Se envía el mismo día en que debe pagar.',
    },
    overdue_reminder: {
      title: 'Aviso de deuda atrasada',
      description: 'Se repite cada pocos días mientras siga sin pagar.',
    },
    statement: {
      title: 'Resumen de deuda',
      description: 'Se envía con el PDF de todo lo que debe y lo que ya pagó.',
    },
  },
  placeholders: {
    name: 'Nombre del cliente',
    business: 'Nombre de tu negocio',
    amount: 'Lo que falta pagar (con S/)',
    date: 'Fecha de pago (o del resumen)',
    description: 'Lo que se llevó',
    daysOverdue: 'Días de atraso',
    paymentLink: 'Enlace para pagar',
    statementUrl: 'Enlace al PDF del resumen de deuda',
  },
  dashboard: {
    title: 'Dashboard',
    subtitle:
      'Los números de {business} en gráficos: lo que cobraste, lo que te deben y lo que viene.',
    answers: {
      collected: 'Del {from} al {to} te pagaron ⟦{amount}⟧ en {count|# pago|# pagos}.',
      more: 'Es ⟦{percent} más⟧ que en el mismo tiempo de antes.',
      less: 'Es ⟦{percent} menos⟧ que en el mismo tiempo de antes.',
      owed: 'Hoy te deben ⟦{amount}⟧ en {count|# deuda|# deudas}, y nadie está atrasado.',
      owedLate: 'Hoy te deben ⟦{amount}⟧. De eso, ⟦{overdue}⟧ ya está atrasado.',
      coming: 'En los próximos 30 días deberían pagarte ⟦{amount}⟧.',
      comingLate:
        'En los próximos 30 días deberían pagarte ⟦{amount}⟧. Además tienes ⟦{overdue}⟧ atrasado por cobrar.',
    },
    tabs: {
      label: 'Reportes',
      sales: 'Ventas',
      purchases: 'Compras',
      collection: 'Lo que cobraste',
      portfolio: 'Lo que te deben',
      projection: 'Lo que vas a cobrar',
      hints: {
        sales: 'Cuánto vendiste, lo más vendido, tus mejores clientes, cómo te pagan y a qué hora.',
        purchases: 'Cuánto compraste, a qué proveedores, qué productos y cómo pagaste.',
        collection: 'Cuánto te pagaron, cómo y quién, en el tiempo que elijas.',
        portfolio: 'Cuánto te deben, desde cuándo y quién te debe más.',
        projection: 'Cuánta plata debería entrar en los próximos días y semanas.',
      },
    },
    sales: {
      answer:
        'En esas fechas vendiste ⟦{total}⟧ en ⟦{sales} ventas⟧, con un ticket promedio de ⟦{average}⟧.',
      answerEmpty: 'En esas fechas no hay ventas. Prueba con otro periodo.',
      vsPrevious: 'vs el periodo anterior',
      kpis: {
        sold: 'Vendiste',
        sales: 'Ventas',
        average: 'Ticket promedio',
        profit: 'Ganancia estimada',
        profitHint: 'Con el costo actual de cada producto.',
        credit: 'Vendido fiado',
      },
      byDay: 'Ventas por día',
      byDayHint: 'Lo que vendiste cada día, al contado y fiado.',
      topProducts: 'Lo más vendido',
      topProductsHint: 'Los 10 productos que más plata te dejaron.',
      topCustomers: 'Tus mejores clientes',
      topCustomersHint: 'Quién te compró más en esas fechas.',
      methods: 'Cómo te pagan',
      methodsHint: 'Efectivo, Yape, Plin, transferencia y lo vendido fiado.',
      categories: 'Por categoría',
      categoriesHint: 'Qué tipo de productos vende más.',
      hours: 'A qué hora vendes',
      hoursHint: 'Tus horas más movidas (la más fuerte resaltada).',
      inSales: '{count|# venta|# ventas}',
      purchases: '{count|# compra|# compras}',
      walkIn: 'Además, {count|# venta|# ventas} a clientes de paso por {amount}.',
      empty: 'No hay ventas en esas fechas.',
      noCustomers: 'Ninguna venta tuvo cliente anotado en esas fechas.',
    },
    purchases: {
      answer: 'En esas fechas compraste ⟦{total}⟧ en ⟦{purchases} compras⟧.',
      answerEmpty: 'En esas fechas no hay compras. Prueba con otro periodo.',
      kpis: {
        bought: 'Compraste',
        purchases: 'Compras',
        average: 'Compra promedio',
        suppliers: 'Proveedores',
      },
      byDay: 'Compras por día',
      byDayHint: 'Lo que gastaste en mercadería cada día.',
      suppliers: 'A quién le compras',
      suppliersHint: 'Tus principales proveedores.',
      products: 'Lo que más compras',
      productsHint: 'Los productos en que más gastaste.',
      methods: 'Cómo pagaste',
      methodsHint: 'De las compras en que anotaste cómo pagaste.',
      count: '{count|# compra|# compras}',
      empty: 'No hay compras en esas fechas.',
      noMethods: 'En esas compras no se anotó cómo pagaste.',
    },
    today: {
      title: 'Para hoy',
      subtitle: 'Lo que necesita tu atención ahora.',
      dueToday: 'Te pagan hoy',
      dueWeek: 'Te pagan esta semana',
      overdue30: 'Atrasadas más de un mes',
      highRisk: 'Clientes que se atrasan mucho',
      receivables: '{count|# deuda|# deudas}',
      customers: '{count|# cliente|# clientes}',
    },
    portfolio: {
      total: 'Total que te deben',
      over90: 'Atrasado más de 3 meses',
      concentration: 'Lo que deben tus 5 mayores deudores',
      concentrationHint: '{amount} entre esos 5 clientes',
      averageDebt: 'Deuda promedio',
      averageDebtHint: 'Por cada deuda sin pagar',
      risk: {
        title: '¿Cómo pagan tus clientes?',
        subtitle: 'Según cómo te pagaron antes.',
        center: 'clientes',
        viewHigh: 'Ver los que se atrasan',
        empty: 'Todavía no hay pagos para saberlo.',
      },
      table: {
        title: 'Tus deudas por estado',
        subtitle: 'Cuántas hay, cuánto sumaban y cuánto falta cobrar.',
        count: 'Cantidad',
        total: 'Monto total',
      },
    },
    concentration: {
      title: '¿A quién cobrarle primero?',
      headline:
        '{count} de tus {total} deudores te deben el {threshold} de todo. Cóbrales primero.',
      subtitle: 'Tus clientes con deuda, del que más debe al que menos.',
      info: 'Ordenamos a tus clientes del que más te debe al que menos. Grupo A: los pocos que juntos te deben casi todo (hasta el 80%): cobrarles a ellos es lo que más plata trae. Grupo B: los siguientes (hasta el 95%). Grupo C: los que deben montos pequeños.',
      empty: 'No tienes clientes con saldo pendiente.',
      axisDebtors: '% de deudores',
      axisDebt: '% del saldo',
      curve: 'Cuánto suman',
      equal: 'Si todos debieran lo mismo',
      tooltipTitle: 'El {share} de tus deudores',
      tooltipDebtors: 'Clientes',
      tooltipDebt: 'Deben',
      classLabel: 'Grupo {key}',
      classes: {
        A: 'Cóbrales primero',
        B: 'Luego estos',
        C: 'Montos pequeños',
      },
      classDebtors: '{count|# cliente|# clientes}',
      classShare: '{share} de lo que te deben',
      table: {
        rank: '#',
        customer: 'Cliente',
        class: 'Grupo',
        outstanding: 'Saldo',
        overdue: 'Vencido',
        receivables: 'Deudas',
        share: '% de lo que te deben',
        cumulative: '% sumado',
      },
      tableNote: 'Los {count} que más te deben. Para la lista completa, usa el botón de descarga.',
      export: 'Descargar la lista para Excel',
    },
    projection: {
      next30: 'Te pagan en los próximos 30 días',
      toRecover: 'Atrasado por cobrar',
      toRecoverHint: '{count|# deuda atrasada|# deudas atrasadas}',
      upcoming: {
        title: 'Próximos pagos',
        subtitle: 'Lo que te deben, de la fecha más cercana a la más lejana.',
        empty: 'No hay pagos por venir.',
        viewAll: 'Ver todas',
      },
    },
    sections: {
      now: 'Estado actual',
      nowHint: 'Cómo están tus cuentas hoy.',
      period: 'Elige las fechas',
      periodHint: 'Lo comparamos con el mismo tiempo de antes.',
      portfolio: 'Lo que te deben',
      portfolioHint: 'Desde cuándo te deben, quién y lo que vas a cobrar.',
    },
    period: {
      presets: {
        thisMonth: 'Este mes',
        lastMonth: 'Mes pasado',
        last30: 'Últimos 30 días',
        thisQuarter: 'Este trimestre',
        last6Months: 'Últimos 6 meses',
        last12Months: 'Últimos 12 meses',
        thisYear: 'Este año',
        lastYear: 'Año pasado',
        custom: 'Personalizado',
      },
      custom: 'Rango personalizado',
      from: 'Desde',
      to: 'Hasta',
      apply: 'Aplicar',
      invalid: 'Revisa las fechas: hasta hoy y como máximo 3 años.',
      previous: 'Periodo anterior',
      next: 'Periodo siguiente',
      granularity: 'Ver por',
      granularities: {
        day: 'Día',
        week: 'Semana',
        month: 'Mes',
      },
      vs: 'Comparado con {range}',
    },
    filters: {
      toggle: 'Filtrar por medio de pago, cliente o día',
      hint: 'También puedes tocar un medio de pago, un día o un cliente en los gráficos de abajo.',
      active: 'Filtrando por',
      method: 'Medio de pago',
      allMethods: 'Todos los medios',
      customer: 'Cliente',
      weekday: 'Día de la semana',
      allDays: 'Todos los días',
      remove: 'Quitar filtro {name}',
      clear: 'Quitar filtros',
      scopeNote:
        'El medio de pago y el día solo cambian los pagos. Lo que fiaste, lo que tocaba cobrar y los mensajes se ven completos.',
      notFiltered: 'No cambia con este filtro',
      customerOnly: 'Filtrado solo por cliente',
      openCustomer: 'Ver cliente',
    },
    analytics: {
      drillHint: 'Haz clic en una barra para ver su detalle.',
      drillBack: 'Volver',
      drillTrail: 'Niveles del detalle',
      export: 'Exportar a Excel (CSV)',
      info: {
        collected:
          'Suma de todos los pagos registrados en el periodo, sin importar cuándo vencía la deuda.',
        issued: 'Todo lo que fiaste en estas fechas.',
        collectionRate:
          'De lo que te tenían que pagar en estas fechas, cuánto ya te pagaron. Mientras más alto, mejor.',
        daysToPay:
          'Cuántos días pasan, en promedio, desde que fías hasta que te pagan. Menos es mejor.',
        payments: 'Cantidad de pagos registrados (un abono parcial cuenta como un pago).',
        newCustomers: 'Clientes creados dentro del periodo.',
        chart:
          'Barras: lo que te pagaron y lo que fiaste. Línea: lo que te tocaba cobrar. Si te pagaron más que la línea, estás recuperando deudas viejas.',
        methods: 'Cómo te pagaron en el periodo, por monto.',
        weekday: 'Qué días de la semana te pagan más: son buenos días para mandar recordatorios.',
        payers: 'Clientes que más pagaron en el periodo.',
        reminders:
          'Los mensajes de WhatsApp que se mandaron y cuántos clientes pagaron dentro de los 7 días siguientes.',
      },
      kpi: {
        collected: 'Cobrado',
        collectedHint: '{count|# pago|# pagos}, de {average} en promedio',
        issued: 'Fiaste',
        issuedHint: '{count|# deuda nueva|# deudas nuevas}',
        collectionRate: '% cobrado a tiempo',
        collectionRateHint: 'De {amount} que te tenían que pagar',
        collectionRateEmpty: 'Nadie tenía que pagarte en estas fechas',
        daysToPay: 'Tardan en pagarte',
        daysToPayValue: '{days|# día|# días}',
        daysToPayHint: 'Desde que fías hasta que te pagan',
        newCustomers: 'Clientes nuevos',
        newCustomersHint: '{total} clientes en total',
        vsPrevious: 'comparado con antes',
        noData: 'Sin datos',
      },
      chartTitle: 'Te pagaron y fiaste',
      chartSubtitle: 'Por {unit}. La línea es lo que te tocaba cobrar.',
      series: {
        collected: 'Te pagaron',
        issued: 'Fiaste',
        due: 'Tocaba cobrar',
        payments: 'Pagos',
      },
      legend: 'Series del gráfico',
      bucket: 'Periodo',
      weekOf: 'Semana del {date}',
      chart: 'Gráfico',
      table: 'Tabla',
      view: 'Vista',
      methods: {
        title: 'Medios de pago',
        subtitle: 'Cómo te pagaron en el periodo.',
        center: 'cobrado',
        empty: 'Sin pagos en el periodo.',
      },
      weekday: {
        title: '¿Qué días te pagan más?',
        subtitle: 'Lo que te pagaron según el día de la semana.',
        best: 'Tu mejor día es el {day}.',
        empty: 'Sin pagos en el periodo.',
      },
      reminders: {
        funnel: 'Mensajes y pagos',
        steps: {
          attempted: 'Se intentó mandar',
          delivered: 'Llegaron',
          paid: 'Pagaron en 7 días',
        },
        title: 'Mensajes por WhatsApp',
        subtitle: 'Los que se mandaron y cuántos clientes pagaron después.',
        sent: 'Llegaron',
        failed: 'No llegaron',
        failedCount: '{count|# no llegó|# no llegaron}',
        effectiveness: 'Pagaron después',
        effectivenessHint: 'Dentro de los 7 días',
        byType: 'Mensajes por tipo',
      },
      payers: {
        title: 'Quién te pagó más',
        subtitle: 'Los clientes que más te pagaron en estas fechas.',
        base: 'Pagado',
        payments: '{count|# pago|# pagos}',
        empty: 'Sin pagos en el periodo.',
      },
    },
    kpi: {
      outstanding: 'Te deben',
      outstandingHint: '{count|# deuda por cobrar|# deudas por cobrar}',
      overdue: 'Atrasado',
      overdueHint: '{rate} de lo que te deben · {count|# deuda|# deudas}',
      collectedMonth: 'Cobrado este mes',
      vsLastMonth: 'vs. mismo periodo del mes anterior',
      dueSoon: 'Te pagan esta semana',
      dueSoonHint: '{count|# deuda|# deudas}',
      highRisk: 'Clientes que se atrasan mucho',
      highRiskHint: 'de {total} clientes',
      collectedAll: 'Cobrado desde el inicio',
      collectedAllHint: '{count|# deuda pagada|# deudas pagadas}',
    },
    trend: {
      title: 'Cobranza mensual',
      subtitle: 'Pagos registrados en los últimos 6 meses.',
      payments: '{count|# pago|# pagos}',
    },
    statusMix: {
      title: 'Tus deudas por estado',
      subtitle: 'Cuántas están pagadas, a tiempo o atrasadas.',
      total: 'Deudas',
    },
    cashFlow: {
      title: 'Plata que debería entrar',
      subtitle: 'Lo que te deben, según la fecha en que te pagan.',
      weekly: 'Semanal',
      monthly: 'Mensual',
      chart: 'Gráfico',
      table: 'Tabla',
      expected: '{amount} en las próximas {count} {unit}',
      weeks: 'semanas',
      months: 'meses',
      overdueNote: 'Ya atrasado (no está en el gráfico): {amount} · {count|# deuda|# deudas}',
      later: 'Más adelante: {amount}',
      weekOf: 'Semana del {date}',
      period: 'Periodo',
      dates: 'Fechas',
      receivables: 'Deudas',
      expectedAmount: 'Te deberían pagar',
    },
    debtors: {
      title: 'Quién te debe más',
      subtitle: 'Los clientes que más te deben.',
      overdue: 'Atrasado',
      current: 'Todavía a tiempo',
      empty: 'Nadie te debe.',
    },
    aging: {
      title: '¿Desde cuándo te deben?',
      subtitle: 'Lo que te deben, según cuántos días de atraso lleva.',
      current: 'A tiempo',
      days1to30: '1–30 días',
      days31to60: '31–60 días',
      days61to90: '61–90 días',
      days90plus: 'Más de 90 días',
      count: '{count|# deuda|# deudas}',
    },
    report: {
      title: 'Resumen del mes',
      subtitle: 'Se prepara solo el último día de cada mes.',
      generate: 'Preparar el de este mes',
      generated: 'Reporte generado.',
      empty: 'Aún no hay reportes',
      emptyDescription: 'El primer reporte se generará al cierre de este mes.',
      collected: 'Te pagaron en el mes',
      pending: 'Te debían al final del mes',
      topOverdue: 'Los más atrasados',
      noOverdue: 'Nadie estaba atrasado.',
      generatedAt: 'Generado el {date}',
    },
  },
  customers: {
    title: 'Clientes',
    subtitle: 'Las personas a las que les fías. Toca una para ver lo que te debe y cobrarle.',
    new: 'Nuevo cliente',
    searchPlaceholder: 'Buscar por nombre o teléfono',
    columns: {
      customer: 'Cliente',
      phone: 'Teléfono',
      risk: '¿Cómo paga?',
      open: 'Por cobrar',
      overdue: 'Atrasadas',
      outstanding: 'Te debe',
      createdAt: 'Registrado',
    },
    documentLabel: 'Doc. {id}',
    empty: 'Aún no tienes clientes',
    emptyDescription: 'Agrega a la primera persona que te compra fiado.',
    emptyFiltered: 'Ningún cliente coincide con los filtros',
    emptyFilteredDescription: 'Prueba con otro nombre o quita el filtro.',
    form: {
      titleNew: 'Nuevo cliente',
      titleEdit: 'Editar cliente',
      name: 'Nombre del cliente',
      phone: 'WhatsApp',
      phoneHint: 'Ej.: 987 654 321. Ahí le llegarán los recordatorios.',
      documentId: 'DNI o RUC',
      documentHint: 'Solo si lo necesitas',
      notes: 'Notas',
      create: 'Crear cliente',
      save: 'Guardar cambios',
    },
    created: 'Cliente creado.',
    updated: 'Cliente actualizado.',
    deleted: 'Cliente eliminado.',
    confirmDeleteTitle: '¿Eliminar cliente?',
    confirmDeleteMessage:
      'Se eliminarán {name} y todas sus deudas, pagos y mensajes. Esta acción no se puede deshacer.',
  },
  customerDetail: {
    noCollections: {
      title: 'Sus compras están en Ventas',
      description: 'Mira todo lo que te compró este cliente.',
      link: 'Ver sus ventas',
    },
    back: 'Clientes',
    newReceivable: 'Anotar fiado',
    viewStatement: 'Ver resumen de deuda',
    sendStatement: 'Mandar resumen por WhatsApp',
    statementChat: 'Abrimos WhatsApp con el mensaje listo',
    statementChatHint: 'Revisa el mensaje en WhatsApp y presiona enviar.',
    statementSent: 'Resumen de deuda enviado a {phone}.',
    statementFailed: 'El resumen se preparó, pero no se pudo enviar.',
    notes: 'Notas',
    kpi: {
      outstanding: 'Te debe',
      outstandingHint: '{count|# deuda por cobrar|# deudas por cobrar}',
      paid: 'Ya te pagó',
      paidHint: 'de {amount} fiado en total',
      onTime: 'Paga a tiempo',
      onTimeHint: 'de {count|# deuda|# deudas}',
      avgLate: 'Se atrasa en promedio',
      avgLateValue: '{days|# día|# días}',
      avgLateHint: '{count|# atrasada|# atrasadas} ahora',
    },
    tabs: {
      receivables: 'Deudas',
      payments: 'Pagos',
      messages: 'Mensajes',
    },
    payments: {
      date: 'Fecha',
      receivable: 'Deuda',
      method: 'Cómo pagó',
      proof: 'Foto',
      amount: 'Monto',
      viewProof: 'Ver',
      empty: 'Todavía no te pagó. Cuando lo haga, toca «Me pagó» en su deuda.',
    },
    messages: {
      date: 'Fecha',
      type: 'Tipo',
      status: 'Estado',
      content: 'Mensaje',
      empty: 'Todavía no se le mandó ningún mensaje.',
    },
  },
  receivables: {
    dueFilter: {
      label: 'Fecha de pago',
      all: 'Todas las fechas',
      presets: {
        today: 'Hoy',
        next7: 'Próximos 7 días',
        next30: 'Próximos 30 días',
        thisMonth: 'Este mes',
        lastMonth: 'Mes pasado',
        past30: 'Últimos 30 días',
      },
      custom: 'Rango personalizado',
      from: 'Desde',
      to: 'Hasta',
      apply: 'Aplicar',
      clear: 'Quitar filtro',
      invalid: 'La fecha inicial debe ser anterior a la final.',
    },
    title: 'Cobranza',
    subtitle: 'Todo lo que fiaste. Toca «Me pagó» cuando te paguen o «WhatsApp» para recordarle.',
    new: 'Anotar fiado',
    searchPlaceholder: 'Buscar cliente o producto',
    methodsCount: '{count} medios',
    filters: {
      open: 'Por cobrar',
      overdue: 'Atrasadas',
      paid: 'Pagadas',
    },
    columns: {
      paymentMethod: 'Cómo pagó',
      customer: 'Cliente',
      description: 'Lo que se llevó',
      dueDate: 'Fecha de pago',
      total: 'Total',
      outstanding: 'Falta pagar',
      status: 'Estado',
      issueDate: 'Fiado el',
    },
    issued: 'Fiado el {date}',
    dueIn: 'Paga en {days|# día|# días}',
    dueTomorrow: 'Paga mañana',
    dueToday: 'Paga hoy',
    overdueBy: 'Atrasada {days|# día|# días}',
    paidPercent: '{percent} pagado',
    actions: {
      pay: 'Me pagó',
      payShort: 'Me pagó',
      remind: 'Recordar por WhatsApp',
      remindShort: 'WhatsApp',
      payLink: 'Copiar enlace para pagar',
      edit: 'Editar',
      delete: 'Eliminar',
    },
    reminderSent: 'Recordatorio enviado a {name} por WhatsApp.',
    reminderOpened: 'Abrimos WhatsApp con el mensaje para {name}: revisa y presiona enviar.',
    reminderFailed: 'No se pudo mandar el recordatorio. Inténtalo otra vez.',
    linkCopied: 'Enlace para pagar {amount} copiado. Pégalo en WhatsApp.',
    linkReady: 'Enlace para pagar: {url}',
    created: 'Fiado anotado.',
    updated: 'Deuda actualizada.',
    deleted: 'Deuda eliminada.',
    confirmDeleteTitle: '¿Eliminar deuda?',
    confirmDeleteMessage:
      'Se eliminará "{description}" junto con sus pagos. Esta acción no se puede deshacer.',
    empty: 'Nadie te debe',
    emptyDescription:
      'Cuando fíes algo, anótalo aquí y Solvia le recordará a tu cliente que te pague.',
    emptyFiltered: 'No hay deudas así',
    emptyFilteredDescription: 'Prueba con otra búsqueda o quita los filtros.',
    clearFilters: 'Quitar filtros',
    form: {
      titleNew: 'Anotar fiado',
      titleEdit: 'Cambiar deuda',
      intro: 'Solo necesitas el cliente, cuánto te debe y para cuándo te paga.',
      customer: '¿Quién te debe?',
      pickCustomer: 'Elige o escribe el nombre del cliente.',
      newCustomer: 'Cliente nuevo',
      searchInstead: 'Buscar otro',
      phoneWhy: 'Su WhatsApp: ahí le llegarán los recordatorios.',
      description: '¿Qué se llevó?',
      descriptionPlaceholder: 'Ej.: arroz, aceite y gaseosa',
      defaultDescription: 'Fiado',
      totalAmount: '¿Cuánto te debe?',
      issueDate: 'Fecha del fiado',
      dueDate: '¿Para cuándo te paga?',
      in7: 'En 1 semana',
      in15: 'En 15 días',
      in30: 'En 1 mes',
      otherDate: 'Otra fecha',
      paysOn: 'Te paga el {date}.',
      soldToday: 'Fiado hoy.',
      changeDate: 'Cambiar fecha',
      create: 'Guardar',
      save: 'Guardar cambios',
    },
  },
  scanner: {
    button: 'Escanear QR',
    open: 'Escanear el QR o el código de barras',
    title: 'Escanear QR',
    hint: 'Apunta al QR o al código de barras: cada producto leído se agrega solo y la cámara sigue lista para el siguiente.',
    starting: 'Encendiendo la cámara…',
    waiting: 'Buscando un código…',
    read: 'Leído: {code}',
    added: '{name} agregado',
    notFound: 'No hay un producto con el código {code}',
    count: '{count|# producto agregado|# productos agregados}',
    done: 'Listo',
    errors: {
      denied:
        'No diste permiso para usar la cámara. Tócalo en el candado junto a la dirección de la página, permite la cámara y vuelve a abrir el escáner.',
      noCamera: 'No encontramos una cámara en este equipo.',
      insecure:
        'La cámara solo funciona en una conexión segura (https) o en este mismo equipo (localhost).',
      failed: 'No se pudo encender la cámara. Ciérrala en otras apps y vuelve a intentarlo.',
    },
  },
  pos: {
    recommended: 'Recomendados · tus {count} más vendidos',
    recommendedChip: 'Recomendados',
    categories: 'Categorías',
    category: '{name} · {count|# producto|# productos}',
    emptyCategory: 'Esta categoría aún no tiene productos.',
    yourProducts: 'Tus productos',
    searchMore: '¿Buscas otro? Escribe su nombre o escanea su código arriba.',
    top: 'Más vendido',
    newTile: 'Producto o servicio nuevo',
    newButton: 'Nuevo',
    info: {
      open: 'Ver detalle',
      openNamed: 'Ver el detalle de {name}',
      alertAt: 'avisar con {count}',
      pack: 'Se compra en saco / caja de',
      sold: 'Últimos 90 días',
      soldCount: '{count|en # venta|en # ventas}',
      addSale: 'Agregar a la venta',
      addPurchase: 'Agregar a la compra',
    },
    newButtonTitle: 'Producto o servicio nuevo (se guarda en tus productos)',
    create: 'Crear «{text}»',
    noCatalog: 'Aún no tienes productos. Crea el primero aquí mismo.',
    newProduct: {
      title: 'Producto nuevo',
      introSale: 'Se guarda en tus productos y se agrega a esta venta.',
      introPurchase: 'Se guarda en tus productos y se agrega a esta compra.',
      priceHint: 'Lo que cobras a tu cliente.',
      costHint: 'Lo que pagaste a tu proveedor por uno.',
      later: 'Luego puedes completarlo (código de barras, aviso de stock) en «Productos».',
      price: 'Precio de venta',
      cost: 'Te costó',
      service: 'Es un servicio (no lleva stock)',
      serviceHint: 'Por ejemplo delivery, instalación o una reparación.',
      create: 'Crear y agregar',
    },
  },
  sales: {
    title: 'Ventas',
    subtitle:
      'Todo lo que vendiste. Toca una venta para ver el detalle, o «Anular» si te equivocaste.',
    new: 'Nueva venta',
    number: 'Venta #{number}',
    walkIn: 'Cliente de paso',
    searchPlaceholder: 'Buscar venta o cliente',
    columns: {
      number: 'Venta',
      customer: 'Cliente',
      items: 'Qué se llevó',
      payment: 'Cómo pagó',
      paymentType: 'Tipo de pago',
      method: 'Medio de pago',
      total: 'Total',
    },
    types: {
      cash: 'Al contado',
      cashHint: 'Te paga ahora',
      credit: 'Fiado',
      creditHint: 'Te paga después',
    },
    docTypes: {
      none: 'Sin comprobante',
      sale_note: 'Nota de venta',
      receipt: 'Boleta',
      invoice: 'Factura',
    },
    pos: {
      shortcuts: 'Atajos de teclado',
      keyCharge: 'Cobrar',
      keySearch: 'Buscar',
      keyBack: 'Volver',
      keyAdd: 'Nuevo',
      results: 'Resultados para «{text}»',
      noCatalog:
        'Aún no tienes productos. Agrégalos en «Productos», o vende un servicio con «Otro producto o servicio».',
      clearSearch: 'Borrar búsqueda',
      left: 'Quedan {count}',
      out: 'Sin stock',
      items: '{count|# producto|# productos}',
      viewTicket: 'Ver venta',
      backToCatalog: 'Volver a los productos',
      ticket: 'Venta',
      emptyTicket: 'Toca un producto o escanea su código para empezar.',
      clear: 'Vaciar',
      clearTitle: '¿Vaciar la venta?',
      clearMessage: 'Se quitarán todos los productos de esta venta.',
      charge: 'Cobrar {amount}',
      back: 'Volver a la venta',
      checkout: 'Cobrar',
      more: 'Comprobante y nota',
      confirm: 'Confirmar venta · {amount}',
      confirmCredit: 'Anotar fiado · debe {amount}',
      leaveTitle: '¿Salir sin guardar?',
      leaveMessage: 'Se perderán los productos que agregaste.',
      leave: 'Salir',
    },
    form: {
      editPrice: 'Cambiar el precio solo en esta venta',
      freeBadge: '(otro)',
      addFree: 'Agregar otro producto o servicio',
      freeName: 'Qué vendes',
      freeNamePlaceholder: 'Ej. delivery, instalación, reparación',
      freePrice: 'Precio',
      freeAdd: 'Agregar',
      byAmount: 'Por monto',
      byAmountPlaceholder: '¿Cuánto?',
      subtotal: 'Subtotal',
      discount: 'Descuento',
      addDiscount: 'Agregar descuento',
      removeDiscount: 'Quitar descuento',
      discountTooHigh: 'El descuento es mayor que la venta.',
      received: '¿Con cuánto paga?',
      paymentMethod: 'Método de pago',
      exact: 'Exacto',
      change: 'Vuelto: {amount}',
      missing: 'Faltan {amount}',
      receivedShort: 'Con lo que paga faltan {amount}. Revisa el monto o elige «Varios».',
      addDownPayment: 'Deja un adelanto',
      downPayment: 'Adelanto',
      downPaymentMethod: '¿Cómo pagó el adelanto?',
      owes: 'Queda debiendo {amount}',
      downTooHigh: 'El adelanto cubre toda la venta: regístrala al contado.',
      addNotes: 'Agregar una nota',
      notes: 'Nota',
      notesPlaceholder: 'Ej. entregar el sábado, garantía de 3 meses…',
      grams: '{count} g',
      milliliters: '{count} ml',
      title: 'Nueva venta',
      search: 'Buscar por código o descripción',
      left: 'Quedan {count}',
      noStock: '(sin stock)',
      onlyLeft: '(solo quedan {count})',
      codeNotFound: 'No encontramos «{code}». Revisa el código o búscalo por nombre.',
      noProducts: 'No hay productos con ese nombre o código.',
      noCatalog: 'Escribe para buscar tus productos.',
      emptyCart: 'Agrega los productos que se lleva tu cliente.',
      empty: 'Agrega al menos un producto.',
      quantity: 'Cantidad',
      less: 'Uno menos',
      more: 'Uno más',
      remove: 'Quitar {name}',
      total: 'Total ({count} productos)',
      customer: 'Cliente',
      howPays: '¿Cómo paga?',
      docType: 'Comprobante',
      docNumber: 'Número',
      addDoc: 'Agregar boleta o nota de venta (opcional)',
      submitCash: 'Cobrar {amount}',
      submitCredit: 'Fiar {amount}',
    },
    done: {
      title: 'Venta #{number} guardada',
      change: 'Dale de vuelto {amount}',
      owes: 'Queda debiendo {amount}',
      print: 'Imprimir ticket',
      whatsapp: 'Enviar por WhatsApp',
      another: 'Nueva venta',
      close: 'Listo',
    },
    ticket: {
      sale: 'Venta #{number}',
      title: 'Ticket de venta N° {number}',
      header: 'TICKET DE VENTA',
      number: 'N° {number}',
      quantity: 'Cant.',
      description: 'Descripción',
      amount: 'Importe',
      items: '{count|# producto|# productos}',
      notReceipt: 'Este ticket no es un comprobante electrónico.',
      greeting: 'Hola {name}, este es el ticket de tu compra:',
      view: 'Ver ticket',
      print: 'Imprimir',
      send: 'WhatsApp',
      download: 'Descargar',
      attachTitle: 'Se descargó el PDF del ticket',
      attachHint: 'En el chat de WhatsApp toca el clip y adjunta el archivo descargado.',
      customer: 'Cliente: {name}',
      subtotal: 'Subtotal',
      discount: 'Descuento',
      total: 'TOTAL',
      paid: 'Pagó con',
      received: 'Recibido',
      change: 'Vuelto',
      downPayment: 'Adelanto',
      owes: 'Queda debiendo',
      thanks: '¡Gracias por tu compra!',
      whatsappOwes: 'Queda pendiente: {amount}.',
    },
    saved: 'Venta #{number} registrada.',
    savedCredit: 'Quedó anotada en «Cobranza».',
    lowStock: 'Te quedan pocos',
    lowStockHint: 'Compra pronto estos productos:',
    lowStockOut: '{name} — sin stock',
    lowStockLeft: '{name} — quedan {count}',
    void: 'Anular venta',
    voidShort: 'Anular',
    viewDetail: 'Ver detalle',
    voidQuestion: '¿Te equivocaste en esta venta?',
    voidHint: 'Anúlala: los productos vuelven al stock y deja de contar en tus ventas.',
    voidHintCredit: 'Anúlala: los productos vuelven al stock y se borra la deuda de «Cobranza».',
    voidTitle: '¿Anular la venta #{number}?',
    voidMessage: 'Los productos vuelven al stock. Esto no se puede deshacer.',
    voidMessageCredit:
      'Los productos vuelven al stock y se borra la deuda de «Cobranza». Esto no se puede deshacer.',
    voided: 'Venta #{number} anulada.',
    shortageBadge: 'Vendido sin stock',
    shortageFilter: 'Sin stock',
    lineShortage: '{count} vendidos sin stock',
    voidedBadge: 'Anulada',
    stillOwes: 'Todavía te debe {amount} de esta venta.',
    paidOff: 'Esta venta fiada ya está pagada.',
    empty: 'Aún no registras ventas',
    emptyDescription: 'Registra tu primera venta: busca el producto, elige cómo paga y listo.',
    emptyFiltered: 'No hay ventas así',
  },
  purchases: {
    title: 'Compras',
    subtitle: 'La mercadería que te llega. Al guardarla, sube tu stock.',
    new: 'Nueva compra',
    ticket: 'Compra N° {number}',
    viewTicket: 'Ver comprobante',
    number: 'Compra #{number}',
    noSupplier: 'Sin proveedor',
    searchPlaceholder: 'Buscar compra o proveedor',
    columns: {
      number: 'Compra',
      supplier: 'Proveedor',
      items: 'Qué llegó',
    },
    pos: {
      keySave: 'Guardar',
      ticket: 'Lo que llegó',
      emptyTicket: 'Toca o escanea los productos que te llegaron.',
      cost: 'Último costo',
      perPack: 'el saco/caja',
      perUnit: 'c/{unit}',
      more: 'Factura y costos',
      viewTicket: 'Ver compra',
      clearTitle: '¿Vaciar la compra?',
      clearMessage: 'Se quitarán todos los productos de esta compra.',
    },
    form: {
      howCounted: 'Cómo lo anotas',
      byPack: 'Sacos/cajas de {size} {unit}',
      byUnit: 'Por {unit}',
      packEquals: '= {quantity} {unit} · a {cost} el {unit}',
      packCost: 'Costo del saco',
      title: 'Nueva compra',
      intro: 'Agrega lo que te llegó, cuántos y cuánto te costó cada uno.',
      emptyCart: 'Busca o escanea los productos que te llegaron.',
      stockAfter: 'Tienes {from} → quedarás con {to}',
      unitCost: 'Costo c/u',
      total: 'Total de la compra',
      paidWith: '¿Cómo pagaste?',
      missingCost: 'Falta el costo de algún producto.',
      supplier: 'Proveedor',
      supplierPlaceholder: 'Escribe el nombre del proveedor…',
      newSupplier: 'Nuevo proveedor: «{name}»',
      noSuppliers: 'Escribe para buscar o crear un proveedor.',
      addDoc: 'Agregar factura o boleta del proveedor (opcional)',
      updateCosts: 'Actualizar el costo de los productos',
      updateCostsHint: 'Así «Ganas» se calcula con lo que pagaste ahora.',
      submit: 'Guardar compra {amount}',
    },
    saved: 'Compra #{number} guardada.',
    savedHint: 'El stock de esos productos ya subió.',
    void: 'Anular compra',
    viewDetail: 'Ver detalle',
    voidQuestion: '¿Te equivocaste en esta compra?',
    voidHint: 'Anúlala: esos productos salen otra vez del stock.',
    voidTitle: '¿Anular la compra #{number}?',
    voidMessage: 'Esos productos salen otra vez del stock. Esto no se puede deshacer.',
    voided: 'Compra #{number} anulada.',
    empty: 'Aún no registras compras',
    emptyDescription: 'Cuando te llegue mercadería, regístrala aquí y tu stock subirá solo.',
    emptyFiltered: 'No hay compras así',
  },
  locations: {
    title: 'Ubicaciones',
    description: 'Sube el plano de tu negocio y marca dónde está cada producto.',
    maps: 'Planos',
    loadError: 'No se pudieron cargar los planos. Revisa tu conexión e inténtalo de nuevo.',
    newMap: 'Nuevo plano',
    renameMap: 'Cambiar nombre',
    mapName: 'Nombre del plano',
    mapNamePlaceholder: 'Ej. Tienda, Almacén, Segundo piso',
    mapImage: 'Imagen del plano',
    mapImageHint:
      'Una foto o dibujo del plano visto desde arriba (JPG, PNG o WEBP). Puedes subirla después.',
    chooseImage: 'Elegir imagen',
    uploadImage: 'Subir imagen',
    changeImage: 'Cambiar imagen del plano',
    removeImage: 'Quitar imagen',
    deleteMap: 'Eliminar plano',
    mapCreated: 'Plano creado',
    mapRenamed: 'Nombre actualizado',
    mapDeleted: 'Plano eliminado',
    confirmDeleteMapTitle: '¿Eliminar este plano?',
    confirmDeleteMapMessage:
      'Se borra «{name}» con {count|# ubicación|# ubicaciones}. Los productos se quedan, sin ubicación.',
    uploading: 'Subiendo el plano…',
    imageUploaded: 'Plano actualizado',
    imageRemoved: 'Imagen quitada',
    imageInvalid: 'No se pudo leer esa imagen. Prueba con una foto JPG o PNG.',
    empty: {
      title: 'Todavía no tienes un plano',
      description:
        'Sube una foto de tu plano o dibuja tu tienda aquí mismo, y marca tus estantes. Así cualquiera encuentra los productos al toque.',
    },
    noImage: {
      title: 'Marca las zonas de tu tienda',
      description:
        'Dibújalas arrastrando sobre la cuadrícula (estantes, refrigeradoras, caja…) o sube la imagen de tu plano.',
    },
    start: {
      title: '¿Cómo quieres armar tu plano?',
      image: {
        title: 'Subir la imagen de mi plano',
        description: 'Una foto o dibujo visto desde arriba. Luego marcas tus estantes encima.',
      },
      draw: {
        title: 'Dibujar mi tienda aquí',
        description:
          'Empiezas con una cuadrícula y dibujas cada zona arrastrando, como al marcar una ubicación.',
      },
      drawNow: 'Crear y empezar a dibujar',
    },
    shape: {
      label: 'Forma de tu local',
      square: 'Cuadrada',
      wide: 'Rectangular',
      long: 'Alargada',
      hint: 'Es solo el contorno: luego dibujas dentro las zonas donde van tus productos.',
    },
    view: { label: 'Vista del plano', flat: '2D', depth: '3D' },
    mark: 'Marcar ubicación',
    cancelMark: 'Cancelar',
    hint: {
      flat: 'Arrastra para mover el plano',
      flatEdit: 'Arrastra una zona para moverla · su esquina para cambiar el tamaño',
      depth: 'Arrastra para girar · rueda o pellizca para acercar',
      mark: 'Arrastra sobre el plano para dibujar la zona (o toca para una pequeña)',
    },
    zoomIn: 'Acercar',
    zoomOut: 'Alejar',
    reset: 'Ver todo el plano',
    rotateLeft: 'Girar a la izquierda',
    rotateRight: 'Girar a la derecha',
    search: '¿Dónde está? Busca un producto o lugar',
    spots: 'Ubicaciones de este plano',
    found: 'Lo encontré aquí',
    noSpots: 'Aún no hay ubicaciones. Toca «Marcar ubicación» y dibuja la zona en el plano.',
    pane: { label: 'Ver', map: 'Plano', list: 'Lista' },
    seeProducts: 'Ver productos',
    noMatches: 'Ningún lugar de este plano tiene «{text}».',
    productsCount: '{count|# producto|# productos}',
    allSpots: 'Todas las ubicaciones',
    spot: {
      new: 'Nueva ubicación',
      edit: 'Editar ubicación',
      name: 'Nombre del lugar',
      namePlaceholder: 'Ej. Estante A, Refrigeradora, Vitrina',
      color: 'Color',
      colorHint: 'Elige uno o toca el recuadro para ver todos los colores.',
      created: 'Ubicación marcada',
      updated: 'Ubicación actualizada',
      moved: 'Ubicación movida',
      deleted: 'Ubicación eliminada',
      confirmDeleteTitle: '¿Eliminar esta ubicación?',
      confirmDeleteMessage:
        'Se borra «{name}» del plano. {count|Su producto se queda|Sus # productos se quedan} sin ubicación.',
      addPlaceholder: 'Agregar producto: código o descripción',
      addHint: 'Escribe el código o el nombre del producto.',
      noResults: 'No hay productos con «{text}».',
      noProducts: 'Todavía no hay productos en este lugar. Búscalos arriba para agregarlos.',
      placed: '«{product}» ahora está en {spot}',
      removed: '«{product}» ya no está en {spot}',
      removeProduct: 'Sacar de aquí',
      seeProduct: 'Ver el detalle del producto',
      detail: 'Ver',
      stock: 'Quedan {count} {unit}',
    },
    product: {
      label: 'Ubicación',
      hint: 'Dónde está en tu plano (Inventario > Ubicaciones).',
      none: 'Sin ubicación',
      see: 'Ver en el plano',
    },
  },
  suppliers: {
    title: 'Proveedores',
    subtitle: 'A quién le compras. Toca uno para cambiar sus datos.',
    new: 'Nuevo proveedor',
    edit: 'Cambiar proveedor',
    searchPlaceholder: 'Buscar por nombre o RUC',
    columns: {
      name: 'Proveedor',
      phone: 'WhatsApp',
      purchases: 'Compras',
    },
    form: {
      name: 'Nombre',
      namePlaceholder: 'Ej.: Distribuidora Lima Norte',
      phone: 'WhatsApp',
      phoneHint: 'Para escribirle con un toque.',
      documentId: 'RUC',
      notes: 'Notas',
      notesPlaceholder: 'Ej.: pasa los martes, pedir con 2 días',
    },
    created: 'Proveedor guardado.',
    updated: 'Proveedor actualizado.',
    deleted: 'Proveedor eliminado.',
    confirmDeleteTitle: '¿Eliminar proveedor?',
    confirmDeleteMessage: 'Se eliminará «{name}».',
    empty: 'Aún no tienes proveedores',
    emptyDescription: 'Agrega a quién le compras (también puedes crearlo al registrar una compra).',
    emptyFiltered: 'No hay proveedores así',
  },
  adjust: {
    open: 'Ajustar stock',
    title: 'Ajustar stock de {name}',
    now: 'Según Solvia hay',
    what: '¿Qué pasó?',
    reasons: {
      count: 'Conté',
      loss: 'Se perdió',
      damage: 'Se dañó',
      correction: 'Corregir',
    },
    quantity: {
      count: '¿Cuántos hay de verdad?',
      loss: '¿Cuántos se perdieron?',
      damage: '¿Cuántos se dañaron?',
      correction: '¿Cuánto sumar o restar?',
    },
    hints: {
      count: 'Escribe lo que contaste; Solvia calcula la diferencia.',
      loss: 'Robados, vencidos o que no aparecen.',
      damage: 'Rotos o que ya no se pueden vender.',
      correction: 'Usa un número negativo para restar (ej.: -2).',
    },
    after: 'Pasará de {from} a {to}.',
    note: 'Nota',
    notePlaceholder: 'Ej.: conteo del lunes, bolsa rota',
    submit: 'Guardar ajuste',
    saved: 'Stock actualizado: ahora hay {count}.',
  },
  kardex: {
    open: 'Ver movimientos',
    title: 'Movimientos de {name}',
    subtitle: 'Cada vez que entró o salió, y con cuánto quedó.',
    now: 'Ahora quedan {count}.',
    balance: 'Quedó',
    shortage: '{count} vendidos sin stock',
    empty: 'Este producto aún no tiene movimientos.',
    newer: 'Más recientes',
    older: 'Más antiguos',
    types: {
      sale: 'Venta',
      sale_void: 'Venta anulada (vuelve al stock)',
      purchase: 'Compra',
      purchase_void: 'Compra anulada (sale del stock)',
      adjustment: 'Ajuste',
    },
    reasons: {
      count: 'Conteo',
      loss: 'Pérdida',
      damage: 'Daño',
      correction: 'Corrección',
    },
  },
  categories: {
    button: 'Categorías',
    title: 'Categorías',
    description:
      'Agrupa tus productos (por ejemplo Bebidas, Limpieza o Abarrotes) para encontrarlos rápido al vender.',
    label: 'Categoría',
    hint: 'Te ayuda a encontrarlo rápido al vender.',
    none: 'Sin categoría',
    all: 'Todas las categorías',
    new: '+ Nueva categoría…',
    newTitle: 'Nueva categoría',
    namePlaceholder: 'Por ejemplo: Bebidas',
    create: 'Crear',
    add: 'Agregar',
    save: 'Guardar',
    rename: 'Cambiar nombre',
    products: '{count|# producto|# productos}',
    empty: 'Aún no tienes categorías. Escribe la primera arriba.',
    created: 'Categoría creada',
    renamed: 'Nombre cambiado',
    deleted: 'Categoría eliminada',
    confirmDeleteTitle: '¿Eliminar la categoría?',
    confirmDeleteMessage: 'Los productos de «{name}» no se borran: se quedan sin categoría.',
  },
  labels: {
    preview: 'Así se imprime',
    sample: 'Ejemplo de producto',
    button: 'Imprimir etiquetas',
    title: 'Imprimir etiquetas QR',
    description:
      'Para pegar en tus productos o en las góndolas. Al escanearlas se agregan a la venta.',
    search: 'Buscar productos…',
    pickAll: 'Seleccionar todos ({count})',
    pickFound: 'Seleccionar los encontrados ({count})',
    unpickAll: 'Quitar la selección',
    chosenOf: '{chosen} de {count}',
    noProducts: 'No hay productos con ese nombre.',
    more: 'Y {count|# más|# más}: búscalos por nombre o usa «Seleccionar todos».',
    copies: 'Cantidad',
    size: 'Tamaño',
    sizes: {
      shelf: 'Góndola',
      shelfHint: '80 × 50 mm · precio grande',
      product: 'Producto',
      productHint: '50 × 30 mm · para pegar',
    },
    paper: 'Dónde imprimes',
    papers: {
      sheet: 'Hoja A4',
      sheetHint: 'Varias etiquetas por hoja, con líneas para recortar (o en hojas adhesivas).',
      roll: 'Impresora de etiquetas',
      rollHint: 'Una etiqueta por página, del tamaño elegido (rollos térmicos).',
    },
    per: 'por {unit}',
    print: 'Imprimir {count|# etiqueta|# etiquetas}',
    tooMany: 'Se imprimen como máximo {count} etiquetas por vez.',
  },
  products: {
    createdCode: 'Su código es {code}. Puedes imprimir su QR desde la lista.',
    kinds: {
      product: 'Producto',
      service: 'Servicio',
      products: 'Productos',
      services: 'Servicios',
    },
    qr: {
      title: 'Código QR',
      description: 'Imprímelo y pégalo en el producto: al escanearlo se agrega a la venta.',
      action: 'Código QR',
      short: 'QR',
      alt: 'Código QR de {name}',
      print: 'Imprimir etiquetas',
      download: 'Descargar imagen',
    },
    title: 'Productos',
    subtitle: 'Lo que vendes, con su precio. Toca uno para cambiarlo.',
    new: 'Nuevo producto',
    searchPlaceholder: 'Buscar por código o descripción',
    filters: {
      label: 'Mostrar',
      active: 'En venta',
      low: 'Por acabarse',
      archived: 'Archivados',
    },
    columns: {
      name: 'Producto',
      unit: 'Se vende por',
      price: 'Precio',
      cost: 'Te cuesta',
      margin: 'Ganas',
      stock: 'Quedan',
      minStock: 'Avisar con',
      category: 'Categoría',
    },
    units: {
      unit: 'Unidad',
      kg: 'Kilo',
      liter: 'Litro',
      box: 'Caja',
      pack: 'Paquete',
      dozen: 'Docena',
      meter: 'Metro',
    },
    unitsShort: {
      unit: 'und.',
      kg: 'kg',
      liter: 'L',
      box: 'cj.',
      pack: 'paq.',
      dozen: 'doc.',
      meter: 'm',
    },
    noStock: 'Sin control',
    archivedBadge: 'Archivado',
    archive: 'Archivar (ya no lo vendo)',
    restore: 'Volver a vender',
    archived: 'Producto archivado.',
    restored: 'El producto vuelve a estar en venta.',
    created: 'Producto guardado.',
    updated: 'Producto actualizado.',
    deleted: 'Producto eliminado.',
    confirmDeleteTitle: '¿Eliminar producto?',
    confirmDeleteMessage: 'Se eliminará «{name}». Si solo ya no lo vendes, mejor archívalo.',
    empty: 'Aún no tienes productos',
    emptyDescription: 'Agrega lo que vendes con su precio para anotar ventas más rápido.',
    emptyFiltered: 'No hay productos así',
    emptyFilteredDescription: 'Prueba con otro nombre o cambia el filtro.',
    form: {
      zoomImage: 'Ver foto en grande',
      changeImage: 'Cambiar foto',
      imageHint: 'JPG, PNG o WEBP · hasta 5 MB',
      qrCode: 'Código del artículo (su QR)',
      qrAuto: 'Al guardarlo, Solvia le crea su código QR único.',
      printLabel: 'Imprimir etiqueta',
      kind: 'Qué es',
      image: 'Foto del producto',
      addImage: 'Agregar foto',
      removeImage: 'Quitar',
      imageTooLarge: 'La foto es muy pesada (máximo 5 MB).',
      servicePlaceholder: 'Ej.: Instalación de cocina',
      serviceCostHint: 'Lo que te cuesta darlo (materiales, pasaje…). Así sabes cuánto ganas.',
      serviceNote: 'Un servicio no lleva stock: se vende las veces que quieras.',
      codeAutoHint:
        'El código de barras del producto. Si lo dejas vacío, Solvia le pone un código único para su QR.',
      codeAutoPlaceholder: 'Se genera solo',
      packSize: 'Lo compras en saco o caja de',
      packSizePlaceholder: 'Ej. 10',
      packSizeHint:
        'Si lo compras en saco o caja, pon cuánto trae ({unit}). Así anotas la compra por sacos y vendes por {unit} o por gramos.',
      titleNew: 'Nuevo producto',
      titleEdit: 'Cambiar producto',
      intro: 'Solo necesitas el nombre y el precio. Lo demás es opcional.',
      name: 'Nombre',
      namePlaceholder: 'Ej.: Arroz Costeño 5 kg',
      price: 'Precio de venta',
      unit: 'Se vende por',
      cost: '¿Cuánto te cuesta?',
      costHint: 'Lo que pagas al proveedor. Sirve para saber cuánto ganas.',
      marginHint: 'Ganas {amount} por cada uno ({percent}).',
      code: 'Código de barras',
      codeHint: 'El número debajo de las barras del empaque. Sirve para buscarlo rápido.',
      trackStock: 'Llevar la cuenta de cuántos me quedan',
      trackStockHint: 'Desmárcalo para cosas que no cuentas, como el pan o un servicio.',
      minStock: 'Avisarme cuando queden',
      minStockHint: 'Te avisaremos para que compres más.',
      create: 'Guardar',
      save: 'Guardar cambios',
    },
  },
  plan: {
    pays: 'Pagas {amount} al mes · {billing}',
    billing: {
      monthly: 'pago mensual',
      annual: 'pago anual',
    },
    title: 'Tu plan este mes',
    plans: {
      free: 'Plan Gratis',
      starter: 'Plan de pago',
      pro: 'Plan de pago',
    },
    automatic: 'Mensajes automáticos',
    automaticHint: 'Los que Solvia envía sola por WhatsApp. Se reinician cada mes.',
    withPacks: 'Incluye {extra} mensajes extra comprados este mes.',
    users: 'Usuarios',
    customers: 'Clientes',
    usedOf: '{used} de {limit}',
    unlimitedUsed: '{used} · ilimitado',
    manual:
      'Los recordatorios que envías tú desde tu celular, con un toque, son gratis y no cuentan aquí.',
    packs:
      '¿Necesitas más mensajes automáticos? Pide un paquete de {size} al equipo de Solvia y se suma a este mes.',
    limitReachedTitle: 'Ya usaste los mensajes automáticos del mes',
    limitReachedBody:
      'Te abrimos WhatsApp para que lo envíes desde tu celular. Puedes pedir un paquete extra.',
    notice: {
      automatic:
        'Ya usaste todos los mensajes automáticos de este mes: los recordatorios ahora se envían desde tu WhatsApp.',
      automaticLow: 'Te quedan {left} mensajes automáticos este mes.',
      users: 'Llegaste al máximo de usuarios de tu plan.',
      customers: 'Llegaste al máximo de clientes de tu plan.',
      link: 'Ver mi plan',
    },
  },
  modules: {
    offer: {
      title: 'Haz crecer tu negocio con Solvia',
      subtitle: 'Suma estos módulos a tu plan. Te los activamos en el día.',
      price: '+{amount} al mes',
      cta: 'Quiero sumarlo',
      note: 'Pago adicional a tu plan. Puedes quitarlo cuando quieras.',
      collections: {
        name: 'Cobranza',
        a: 'Anota lo que te deben y quién te pagó',
        b: 'Recordatorios por WhatsApp y estados de cuenta',
        c: 'Dashboard y reporte de cobros',
      },
      sales: {
        name: 'Ventas',
        a: 'Registra ventas en segundos, al contado o fiadas',
        b: 'Escanea el código de barras de tus productos',
        c: 'Reportes de ventas por cliente y producto',
      },
      inventory: {
        name: 'Inventario',
        a: 'Tu stock se actualiza solo con cada venta y compra',
        b: 'Compras y proveedores en un solo lugar',
        c: 'Te avisa lo que se está acabando',
      },
    },
    off: {
      title: 'Esta sección no está activa',
      description:
        'Tu negocio aún no tiene activado este módulo. Puedes sumarlo a tu plan cuando quieras.',
    },
  },
  split: {
    option: 'Varios',
    method: 'Método',
    add: 'Otro método',
    remove: 'Quitar este método',
    complete: 'Completo',
    missing: 'Falta {amount}',
    over: 'Te pasaste por {amount}',
    sum: 'Suma {amount}',
    hint: 'Por ejemplo, una parte en efectivo y el resto con Yape.',
    notComplete: 'Los montos no suman {amount}. Revísalos.',
    toggle: 'Pagó con varios métodos',
    single: 'Pagó con un solo método',
    paidWith: 'Pagó con',
    youPaidWith: 'Pagaste con',
  },
  payment: {
    title: 'Me pagó',
    noAmount: 'Escribe cuánto te pagó.',
    tooMuch: 'Es más de lo que debe ({amount}).',
    outstanding: 'Falta pagar',
    ofTotal: 'de {total}',
    due: 'Paga el {date}',
    amount: '¿Cuánto te pagó?',
    full: 'Te pagó todo',
    partial: 'Te pagó una parte',
    payAll: 'Pagó todo ({amount})',
    date: 'Fecha del pago',
    paidToday: 'Pagó hoy.',
    method: '¿Cómo te pagó?',
    proof: 'Foto de la captura',
    proofHint: 'Una foto o captura de Yape, Plin o la transferencia.',
    proofTooLarge: 'La foto es muy pesada (máximo 5 MB).',
    proofSelect: 'Agregar foto',
    proofRemove: 'Quitar',
    statementNote:
      'Después de guardar, tu cliente recibe por WhatsApp un resumen de lo que aún debe.',
    submit: 'Guardar pago',
    success: 'Pago de {amount} registrado.',
  },
  settings: {
    title: 'Configuración',
    subtitle: 'Cuándo y qué mensajes de WhatsApp le llegan a tus clientes, y quién usa Solvia.',
    adminOnly: 'Solo un administrador puede modificar esta sección.',
    tabs: {
      plan: 'Mi plan',
      reminders: 'Recordatorios',
      templates: 'Textos de los mensajes',
      log: 'Mensajes enviados',
      users: 'Usuarios',
      preferences: 'Preferencias',
      business: 'Empresa',
    },
    reminders: {
      title: 'Recordatorios automáticos',
      description:
        'Solvia revisa tus deudas cada hora y le escribe a tu cliente por WhatsApp según lo que elijas aquí. Nunca manda el mismo aviso dos veces.',
      enabled: 'Mandar recordatorios solos',
      daysBefore: '¿Cuántos días antes le aviso?',
      daysBeforeHint: 'Pon 0 si no quieres avisar antes',
      overdueEvery: 'Si se atrasa, ¿cada cuántos días le vuelvo a escribir?',
      overdueEveryHint: 'Pon 0 si no quieres insistir',
      onDueDate: 'Avisarle también el mismo día que debe pagar',
      save: 'Guardar',
      saved: 'Guardado.',
      runNow: 'Mandar recordatorios ahora',
      runResult: 'Listo: {sent} enviados, {failed} no llegaron, {evaluated} deudas revisadas.',
      timeline: 'Así se verá con tus reglas',
      timelineBefore: '{days|# día|# días} antes',
      timelineDue: 'El día que debe pagar',
      timelineOverdue: 'Cada {days|día|# días} después',
      timelineOff: 'Desactivado',
    },
    templates: {
      title: 'Textos de los mensajes',
      description:
        'Escribe los mensajes a tu manera. Toca un dato (como el nombre o el monto) para agregarlo: Solvia lo completa con los datos de cada cliente.',
      defaultText: 'Texto original',
      preview: 'Así le llegará',
      restore: 'Volver al texto original',
      save: 'Guardar',
      saved: 'Mensaje guardado.',
      restored: 'Volvimos al texto original.',
      variables: 'Datos que puedes agregar',
    },
    log: {
      title: 'Mensajes enviados',
      date: 'Fecha',
      customer: 'Cliente',
      type: 'Tipo',
      status: 'Estado',
      content: 'Mensaje',
      sent: 'Llegó',
      failed: 'No llegó',
      empty: 'Todavía no se mandó ningún mensaje.',
    },
    preferences: {
      title: 'Preferencias',
      description: 'Se guardan en este navegador.',
      language: 'Idioma en este equipo',
      theme: 'Apariencia',
    },
    business: {
      title: 'Idioma de la empresa',
      description:
        'El idioma en que Solvia se muestra a todo tu equipo y en que se escriben los tickets, reportes y mensajes de WhatsApp para tus clientes. Cada persona puede cambiarlo solo en su equipo con el botón ES / EN de arriba.',
      saved: 'Idioma de la empresa guardado',
    },
  },
  help: {
    title: 'Centro de ayuda',
    subtitle: 'Aprende a usar Solvia paso a paso.',
    tourTitle: 'Recorrido guiado',
    tourDescription:
      'Te mostramos dónde está cada cosa, sobre la pantalla de verdad. Dura unos 2 minutos.',
    startTour: 'Iniciar recorrido',
    stepLabel: 'Paso {number}',
    guides: {
      start: {
        title: 'Primeros pasos',
        step1: 'Entra con tu correo y contraseña.',
        step2: 'Toca el botón «+ Anotar» y elige «Anotar fiado».',
        step3: 'Escribe el nombre del cliente, cuánto te debe y para cuándo te paga. ¡Listo!',
      },
      customers: {
        title: 'Agregar clientes',
        step1:
          'Toca «+ Anotar» y elige «Nuevo cliente» (o escribe un nombre nuevo al anotar un fiado).',
        step2: 'Escribe su nombre y su WhatsApp, por ejemplo 987 654 321.',
        step3: 'En Clientes, toca a alguien para ver lo que te debe y lo que te pagó.',
      },
      receivables: {
        title: 'Anotar un fiado',
        step1: 'Toca «+ Anotar» y elige «Anotar fiado».',
        step2: 'Busca al cliente, escribe cuánto te debe y elige para cuándo te paga.',
        step3: 'En «Cobranza» ves todo lo que falta cobrar; arriba puedes ver solo las atrasadas.',
      },
      payments: {
        title: 'Cuando te pagan',
        step1: 'En la deuda, toca «Me pagó» (o «+ Anotar» → «Registrar pago»).',
        step2: 'Escribe cuánto te pagó y cómo (Yape, Plin, efectivo…). La foto es opcional.',
        step3: 'Solvia descuenta la deuda sola.',
        step4: 'Para recordarle a alguien que te pague, toca «WhatsApp» en su deuda.',
      },
      reminders: {
        title: 'Recordatorios automáticos',
        step1: 'Solvia revisa tus deudas cada hora.',
        step2:
          'Le escribe a tu cliente unos días antes, el día que debe pagar y, si se atrasa, cada pocos días.',
        step3: 'Puedes ver todos los mensajes en Configuración > Mensajes enviados.',
      },
      dashboard: {
        title: 'Entender los reportes',
        step1: 'En Inicio ves cuánto te deben, cuánto te pagaron este mes y a quién cobrarle hoy.',
        step2:
          'En Reportes ves los números con más detalle: por mes, por cliente y por medio de pago.',
        step3: '«¿A quién cobrarle primero?» te muestra los pocos clientes que te deben casi todo.',
      },
    },
    helloHint: 'Te acompaño mientras aprendes a cobrar sin perseguir a nadie.',
    searchPlaceholder: 'Busca una guía o una pregunta…',
    noResults: 'No encontramos nada con "{query}". Prueba con otra palabra.',
    guidesTitle: 'Guías rápidas',
    guidesSubtitle: 'Todo lo que necesitas, en pocos pasos.',
    stepsCount: '{count|# paso|# pasos}',
    faqSubtitle: 'Respuestas cortas a las dudas más comunes.',
    clearSearch: 'Limpiar búsqueda',
    stuckTitle: '¿Sigues con dudas?',
    stuckDescription: 'Haz el recorrido guiado: te mostramos cada pantalla en 2 minutos.',
    faqTitle: 'Preguntas frecuentes',
    faq: {
      q1: '¿Cómo sabe Solvia si un cliente paga bien?',
      a1: 'Mira cuántas veces te pagó a tiempo, cuántos días suele atrasarse y si ahora tiene deudas atrasadas.',
      q2: '¿Se puede enviar un recordatorio dos veces?',
      a2: 'El envío automático no repite un recordatorio en el mismo periodo. Si lo necesitas, puedes enviarlo manualmente desde la fila de la deuda.',
      q3: '¿Qué pasa si registro un pago parcial?',
      a3: 'La deuda baja, queda como «Pagó una parte» y los recordatorios siguen hasta que termine de pagar.',
      q4: '¿Quién puede cambiar la configuración?',
      a4: 'Solo los administradores. Los cobradores pueden anotar clientes, deudas y pagos.',
      q5: '¿Puedo usar Solvia desde el celular?',
      a5: 'Sí. Funciona en el navegador de tu celular y puedes agregarla a tu pantalla como una app.',
      q6: '¿Por qué se abre WhatsApp al tocar «WhatsApp»?',
      a6: 'Solvia abre el chat de tu cliente con el mensaje ya escrito: solo revísalo y presiona enviar.',
      q7: '¿En qué idioma se envían los mensajes y el PDF?',
      a7: 'En el idioma que tengas elegido arriba. Si cambiaste el texto de un mensaje, se manda tal como lo escribiste.',
      q8: '¿Qué significan los colores de los clientes?',
      a8: 'Verde: paga bien. Amarillo: a veces se atrasa. Rojo: se atrasa mucho, conviene cobrarle primero.',
    },
  },
  assistant: {
    name: 'Bowl',
    open: 'Abrir a Bowl, tu asistente',
    role: 'Tu ayudante en Solvia',
    greeting: '¡Hola, {name}! Soy Bowl.',
    intro: 'Te ayudo a usar Solvia. ¿Qué necesitas?',
    tour: 'Hacer el recorrido guiado',
    helpCenter: 'Ir al centro de ayuda',
    comingSoon: 'Muy pronto podrás chatear conmigo aquí.',
  },
  tour: {
    sectionButton: 'Recorrido',
    sectionButtonTitle: '¿Cómo se usa {name}? Te lo muestro paso a paso',
    sectionTitle: 'Recorridos por sección',
    sectionHint:
      '¿Olvidaste cómo se hace algo? Elige la sección y te la muestro paso a paso. También está el botón «Recorrido» arriba en cada pantalla.',
    general: 'Recorrido general',
    thisSection: 'Recorrido de esta sección ({name})',
    names: {
      home: 'Inicio',
      sales: 'Ventas',
      salePos: 'Nueva venta',
      purchases: 'Compras',
      purchasePos: 'Nueva compra',
      customers: 'Clientes',
      customerDetail: 'Detalle del cliente',
      receivables: 'Cobranza',
      products: 'Productos',
      suppliers: 'Proveedores',
      locations: 'Ubicaciones',
      dashboard: 'Dashboard',
      reports: 'Reportes',
      settings: 'Configuración',
      help: 'Ayuda',
    },
    sections: {
      home: {
        actions: {
          title: 'Lo de todos los días',
          body: 'Desde aquí vendes, anotas un fiado, registras un pago o agregas un cliente, con un toque.',
        },
        quickAdd: {
          title: 'Siempre a mano',
          body: 'El botón «+ Anotar» está en todas las pantallas: úsalo cuando vendas, fíes o te paguen.',
        },
        today: {
          title: 'A quién cobrarle hoy',
          body: 'Los atrasados y los que te pagan pronto. Toca «Me pagó» o «WhatsApp» para recordarles.',
        },
        navigation: {
          title: 'El menú',
          body: 'Cada sección de tu negocio: ventas, cobranza, inventario y resultados.',
        },
      },
      sales: {
        intro: {
          title: 'Tus ventas',
          body: 'Aquí ves todas tus ventas: al contado y al fiado, con su ticket.',
        },
        new: { title: 'Vender', body: '«Nueva venta» abre la pantalla de venta (atajo Alt+A).' },
        search: { title: 'Buscar', body: 'Escribe el número de venta o el nombre del cliente.' },
        filters: {
          title: 'Filtrar',
          body: 'Muestra solo las ventas al contado, al fiado o sin stock. En el celular se abren con este botón.',
        },
        table: {
          title: 'El detalle',
          body: 'Toca una venta para ver qué se llevó, cómo pagó y su ticket: verlo, imprimirlo, enviarlo por WhatsApp o descargarlo.',
        },
        columns: {
          title: 'Columnas',
          body: 'Elige qué columnas ver. Toca el título de una columna para ordenar.',
        },
      },
      salePos: {
        intro: {
          title: 'Pantalla de venta',
          body: 'Arma el ticket a la izquierda y cobra a la derecha. «Ventas» arriba te regresa a la lista.',
        },
        search: {
          title: 'Buscar el producto',
          body: 'Por código o descripción (Alt+B). Con un lector de códigos, escanea y se agrega solo.',
        },
        scan: {
          title: 'Escanear con la cámara',
          body: 'Apunta la cámara al QR del producto: se agrega y puedes seguir escaneando otros.',
        },
        new: {
          title: 'Producto nuevo',
          body: 'Si no está en tu catálogo, créalo aquí mismo sin salir de la venta (Alt+A).',
        },
        categories: {
          title: 'Categorías',
          body: '«Recomendados» son tus más vendidos; toca una categoría para ver sus productos.',
        },
        catalog: {
          title: 'Toca para agregar',
          body: 'Cada toque suma uno al ticket. La «i» muestra el detalle: precio, stock, dónde está y su foto.',
        },
        ticket: {
          title: 'El ticket',
          body: 'Cambia cantidades, el precio de esta venta o un descuento. Luego «Cobrar»: al contado o fiado, y cómo pagó.',
        },
        keys: { title: 'Atajos', body: 'Alt+S cobra, Alt+B busca, Alt+A crea y Esc regresa.' },
      },
      purchases: {
        intro: {
          title: 'Tus compras',
          body: 'Lo que le compras a tus proveedores: el stock sube y los costos se actualizan.',
        },
        new: { title: 'Comprar', body: '«Nueva compra» abre la pantalla de compra (atajo Alt+A).' },
        search: { title: 'Buscar', body: 'Escribe el número de compra o el proveedor.' },
        filters: {
          title: 'Filtrar',
          body: 'Elige qué compras ver. En el celular se abren con este botón.',
        },
        table: {
          title: 'El detalle',
          body: 'Toca una compra para ver lo que trajo, cómo pagaste y su comprobante en PDF.',
        },
        columns: {
          title: 'Columnas',
          body: 'Elige qué columnas ver. Toca el título de una columna para ordenar.',
        },
      },
      purchasePos: {
        intro: {
          title: 'Pantalla de compra',
          body: 'Arma la compra a la izquierda y guárdala a la derecha.',
        },
        search: {
          title: 'Buscar el producto',
          body: 'Por código o descripción (Alt+B), o escanéalo con el lector.',
        },
        scan: {
          title: 'Escanear con la cámara',
          body: 'Apunta la cámara al QR: se agrega y sigues con el siguiente.',
        },
        new: {
          title: 'Producto nuevo',
          body: 'Si es algo que nunca compraste, créalo aquí mismo (Alt+A).',
        },
        catalog: {
          title: 'Toca para agregar',
          body: 'Cada toque suma uno. La «i» muestra el detalle del producto.',
        },
        ticket: {
          title: 'La compra',
          body: 'Pon cantidades (o sacos), el costo, el proveedor y cómo pagaste. Luego guarda (Alt+S).',
        },
        keys: { title: 'Atajos', body: 'Alt+S guarda, Alt+B busca, Alt+A crea y Esc regresa.' },
      },
      customers: {
        intro: { title: 'Tus clientes', body: 'Cuánto te debe cada uno y si paga bien.' },
        new: { title: 'Nuevo cliente', body: 'Agrega un cliente con su WhatsApp (atajo Alt+A).' },
        search: { title: 'Buscar', body: 'Por nombre, documento o teléfono.' },
        filters: { title: 'Filtrar', body: 'Por ejemplo, solo los que te deben.' },
        table: {
          title: 'El detalle',
          body: 'Toca un cliente para ver sus deudas, pagos y su estado de cuenta. Clic derecho para editarlo.',
        },
        columns: {
          title: 'Columnas',
          body: 'Elige qué columnas ver. Toca el título para ordenar.',
        },
      },
      customerDetail: {
        intro: {
          title: 'El cliente',
          body: 'Todo lo de este cliente: lo que te debe, lo que te pagó y su historial.',
        },
        actions: {
          title: 'Acciones',
          body: 'Anota un fiado o un pago, recuérdale por WhatsApp o mira su estado de cuenta en PDF.',
        },
      },
      receivables: {
        intro: { title: 'Cobranza', body: 'Lo que te deben: cada fiado con su vencimiento.' },
        new: {
          title: 'Anotar un fiado',
          body: 'Registra lo que te quedaron debiendo (atajo Alt+A).',
        },
        search: { title: 'Buscar', body: 'Por cliente o descripción.' },
        filters: { title: 'Filtrar', body: '«Todos», pendientes, atrasados o pagados.' },
        table: {
          title: 'Cobrar desde aquí',
          body: '«Me pagó» cuando te pagan (puede ser con varios métodos); «WhatsApp» para recordarle.',
        },
        columns: {
          title: 'Columnas',
          body: 'Elige qué columnas ver. Toca el título para ordenar.',
        },
      },
      products: {
        intro: {
          title: 'Tus productos',
          body: 'Lo que vendes, con su precio, costo, stock y código QR.',
        },
        actions: {
          title: 'Acciones',
          body: '«Nuevo producto» (Alt+A), «Categorías» para agruparlos e «Imprimir etiquetas» con su QR.',
        },
        search: { title: 'Buscar', body: 'Por código o descripción.' },
        filters: {
          title: 'Filtrar',
          body: 'En venta, por acabarse, archivados, productos o servicios y por categoría.',
        },
        table: {
          title: 'Cambiar un producto',
          body: 'Toca uno para cambiarlo: precio, foto, categoría, ubicación y su etiqueta.',
        },
        columns: {
          title: 'Columnas',
          body: 'Elige qué columnas ver. Toca el título para ordenar.',
        },
      },
      suppliers: {
        intro: { title: 'Tus proveedores', body: 'A quién le compras, con su RUC y WhatsApp.' },
        new: { title: 'Nuevo proveedor', body: 'Agrégalo aquí (atajo Alt+A).' },
        search: { title: 'Buscar', body: 'Por nombre o RUC.' },
        filters: { title: 'Filtrar', body: 'Elige qué proveedores ver.' },
        table: {
          title: 'Cambiar un proveedor',
          body: 'Toca uno para editarlo; con el menú «…» le escribes por WhatsApp.',
        },
        columns: { title: 'Columnas', body: 'Elige qué columnas ver.' },
      },
      locations: {
        intro: {
          title: 'Ubicaciones',
          body: 'El plano de tu negocio: marca dónde está cada producto para encontrarlo al toque.',
        },
        new: {
          title: 'Nuevo plano',
          body: 'Sube la imagen de tu plano o dibuja tu tienda aquí mismo (atajo Alt+A).',
        },
        panes: {
          title: 'Plano o lista',
          body: 'En el celular cambia entre ver el plano y la lista de ubicaciones.',
        },
        mark: {
          title: 'Marcar una zona',
          body: 'Toca aquí y arrastra sobre el plano para dibujar un estante, una refrigeradora o la caja.',
        },
        view: {
          title: '2D o 3D',
          body: 'En 2D mueves y cambias el tamaño de las zonas; en 3D las ves levantadas con las fotos de sus productos.',
        },
        stage: {
          title: 'Mover el plano',
          body: 'Arrastra para moverlo o girarlo; la rueda o dos dedos para acercar. Toca una zona para elegirla.',
        },
        panel: {
          title: '¿Dónde está?',
          body: 'Busca un producto y se marca su zona. En una zona agregas sus productos y tocas uno para ver su detalle.',
        },
      },
      dashboard: {
        intro: {
          title: 'Dashboard',
          body: 'Tus números en gráficos: cuánto vendes, compras, cobras y te deben.',
        },
        views: {
          title: 'Vistas',
          body: 'Cada pestaña responde una pregunta: ventas, compras, cobranza, quién te debe y lo que vas a cobrar.',
        },
        period: {
          title: 'El periodo',
          body: 'Elige las fechas y cómo agruparlas (día, semana o mes).',
        },
      },
      reports: {
        intro: {
          title: 'Reportes',
          body: 'Tablas con el detalle de tu negocio, para ver o descargar.',
        },
        chooser: {
          title: 'Elegir el reporte',
          body: 'Primero el área (Ventas, Compras, Cobranza, Inventario) y luego el reporte.',
        },
        toolbar: {
          title: 'Fechas y archivos',
          body: 'Elige las fechas y abre el reporte en PDF o Excel para guardarlo o enviarlo.',
        },
        search: {
          title: 'Buscar en el reporte',
          body: 'Escribe para quedarte solo con las filas que te interesan.',
        },
        table: { title: 'El reporte', body: 'Toca el título de una columna para ordenar.' },
      },
      settings: {
        intro: {
          title: 'Configuración',
          body: 'Los datos de tu negocio, tu equipo y cómo funciona Solvia para ti.',
        },
        tabs: {
          title: 'Secciones',
          body: 'Empresa (idioma), mensajes y recordatorios, tu plan, usuarios y preferencias.',
        },
        reminders: {
          title: 'Recordatorios solos',
          body: 'Elige cuándo Solvia les recuerda a tus clientes por WhatsApp.',
        },
      },
      help: {
        intro: {
          title: 'Ayuda',
          body: 'Preguntas frecuentes y los recorridos de cada sección, cuando los necesites.',
        },
      },
    },
    welcomeTitle: 'Bienvenido a Solvia',
    welcomeBody: '¿Te muestro rápido cómo funciona? Son menos de 2 minutos.',
    start: 'Comenzar recorrido',
    later: 'Ahora no',
    next: 'Siguiente',
    previous: 'Anterior',
    finish: 'Finalizar',
    skip: 'Salir del recorrido',
    progress: '{current} de {total}',
    steps: {
      sectionTours: {
        title: 'Recorrido de cada sección',
        body: 'En cada pantalla, este botón te muestra paso a paso cómo se usa. Úsalo cuando olvides cómo hacer algo.',
      },
      homeActions: {
        title: 'Lo de todos los días',
        body: 'Desde aquí anotas un fiado, registras un pago o agregas un cliente.',
      },
      quickAdd: {
        title: 'Siempre a mano',
        body: 'Este botón «+ Anotar» está en todas las pantallas: úsalo cuando fíes algo o te paguen.',
      },
      homeToday: {
        title: 'A quién cobrarle hoy',
        body: 'Aquí aparecen los atrasados y los que te pagan pronto. Toca «Me pagó» o «WhatsApp».',
      },
      navigation: {
        title: 'Menú',
        body: 'Inicio, lo que te deben, tus clientes y los reportes.',
      },
      rowActions: {
        title: 'Cobra desde aquí',
        body: '«Me pagó» cuando te pagan; «WhatsApp» para recordarle a tu cliente.',
      },
      customersTable: {
        title: 'Tus clientes',
        body: 'Lo que te debe cada uno y si paga bien. Toca a alguien para ver su detalle.',
      },
      kpis: {
        title: 'Reportes',
        body: 'Cuánto te deben, cuánto te pagaron y más números, cuando los necesites.',
      },
      reminderRules: {
        title: 'Recordatorios solos',
        body: 'Elige cuándo Solvia le escribe a tu cliente: antes, el mismo día y si se atrasa.',
      },
      help: {
        title: 'Bowl te ayuda',
        body: 'Toca a Bowl, nuestro búho, para repetir este recorrido o ver la ayuda.',
      },
    },
  },
  notFound: {
    title: 'Página no encontrada',
    back: 'Ir al inicio',
  },
};

/** Recursively widens string literals so other locales can provide different text with the same shape. */
type DeepStrings<T> = { [K in keyof T]: T[K] extends string ? string : DeepStrings<T[K]> };
export type Messages = DeepStrings<typeof es>;
